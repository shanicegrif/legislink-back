...
representatives.get("/ocd/:id", async (req, res) => {
    const { id } = req.params;
    const representativesData = await getSingleRepresentativeByOcdID(id)
    console.log(representativesData)
    if(representativesData){
        //no query, show everything
        res.status(200).json({ success: true, data: { payload: [...representativesData] } });
...
representatives.get("/:id", async (req, res) => {
    const { id } = req.params;
    const representativesData = await getSingleRepresentativeByBioguidId(id);
    console.log(representativesData)
    if(representativesData){
        //no query, show everything
        res.status(200).json({ success: true, data: { payload: [...representativesData] } });
